...
\begin{verbatim}
  ...
  Ull  r16, r17, r18, r19, r20, r21, r22, r23, r24, r25, r26, r27, r28, r29, r30, r31;
  Ull  cc0, cc1, cc2, cc3, ex0, ex1;
  Ull  J[16], K[16], IDX[16]; /* log2(NumSamples=65536)=16¤Þ¤ÇÂÐ±þ²Ä */
  Ull  BufReal[16], BufImag[16];
  Ull  ar, ai, rok, iok, roj, ioj, tr0, ti0, tr1, ti1;
  Ull  Pipeline, Lmmrotate; /* log2(NumSamples=65536)=16²ó·«¤êÊÖ¤¹¤È,ºÇ½ªÃÊ¤ÎLMM¤Ë,ºÇ½é¤ÎRealOut/ImagOut¤¬³ÊÇ¼¤µ¤ì¤ë */
  printf("<<<IMAX>>> NumSamples=%d (LMM should be >= %dB)\n", NumSamples, NumSamples*4*2);
  reset_nanosec();
...
Ull  Buf[32];
Ull  base, J[16], K[16];  /* log2(NumSamples=65536)=16¤Þ¤ÇÂÐ±þ²Ä */
Ull  Pipeline, Lmmrotate; /* log2(NumSamples=65536)=16²ó·«¤êÊÖ¤¹¤È,ºÇ½ªÃÊ¤ÎLMM¤Ë,ºÇ½é¤ÎOut¤¬³ÊÇ¼¤µ¤ì¤ë */
...
#define sort_core1(r, rp1, x, In, Ilen, BE8, Buf, Out, Olen) \
 ...
 mop(OP_LDR, 3,   &BR[r][3][1], J[x],        base,      MSK_D0,      In,         Ilen, 0, 0,      NULL,   Ilen);/*LMM[2]³ÎÄê   LD¼Â¹Ô¤Ïcol2*//* stage#3 */\
 mop(OP_LDR, 3,   &BR[r][3][0], K[x],        base,      MSK_D0,      In,         Ilen, 0, 0,      NULL,   Ilen);/*LMM[1]´Ö¼Ú¤ê LD¼Â¹Ô¤Ïcol2*//* stage#3 */\
 ...
 mop(OP_STR, ex0, &BR[r][3][1], Buf,         L8,        MSK_W0,      Out,        Olen, 0, 0,      NULL,   Olen);/*LMM[1]´Ö¼Ú¤ê LD¼Â¹Ô¤Ïcol2*//* stage#5 */\
 cex(OP_CEXE,     &ex1,         0, cc2, cc1, cc0, 0x004c);                     /* if ((!cc2 && cc1 && cc0) || ( cc1 && !cc0)) 6,3,2 0x004c *//* stage#5 */\
 mop(OP_STR, ex1, &BR[r][3][0], Buf,         L8,        MSK_W0,      Out,        Olen, 0, 0,      NULL,   Olen) /*LMM[1]´Ö¼Ú¤ê LD¼Â¹Ô¤Ïcol2*//* stage#5 */
...
//EMAX5A begin search mapdist=0
    for (CHIP=0; CHIP<NCHIP; CHIP++) { //¥Á¥Ã¥×Êý¸þ¤Ï¸¡º÷ÂÐ¾ÝÊ¸½ñ¤ÎÂç¤­¤µÊý¸þ
      for (INIT0=1,LOOP0=loop,dmy=0; LOOP0--; INIT0=0) { //Ä¹¤µ¤Ï64KBÊ¸»ú¤Þ¤Ç
      /* map#0 */
...
//EMAX5A begin jacobi mapdist=0 /* 7 PAD>0¤Î¾ì¹ç,PLOAD¤ÈLOADÎÎ°è¤¬°ìÉô½ÅÊ£.loadÃæ¤ÎLMM¤Ë¤âPLOAD¤ò¼è¤ê¹þ¤à¤¿¤á¤Ë½ÂÂÚ¤¬È¯À¸¤¹¤ë */
      for (CHIP=0; CHIP<NCHIP; CHIP++) { /* output channels are parallelized by multi-chip (OC/#chip) */
   /*2*/for (INIT1=1,LOOP1=RMGRP,roofs=0-WD*4; LOOP1--; INIT1=0) {      /* stage#0 *//* mapped to FOR() on BR[63][1][0] */
     /*1*/for (INIT0=1,LOOP0=WD-PAD*2,coofs=(PAD-1)*4; LOOP0--; INIT0=0) {          /* stage#0 *//* mapped to FOR() on BR[63][0][0] */
            exe(OP_ADD,  &coofs, INIT0?coofs:coofs, EXP_H3210, 4, EXP_H3210, 0LL, EXP_H3210, OP_AND, 0x00000000ffffffffLL, OP_NOP, 0LL); /* stage#0 */
...
excluding the CGRA starting time ¡ÊK$^2$xREP cycles¡Ë and LMM replacement
...
imax() {
  ...
  for (top=0; top<M1/NCHIP; top+=RMGRP) { /* will be parallelized by multi-chip (M/#chip) */
    for (blk=0; blk<L; blk+=H) { /* 3½Å¥ë¡¼¥×ÌÜ (C¤¬³ÎÄê¤¹¤ë¤Þ¤Ç¤ÎDMAÆþ¤ì´¹¤¨¤ÏR/W¤òÈ¼¤¦¤¿¤á¥ª¡¼¥Ð¥Ø¥Ã¥É¤Ë¤Ê¤ë. B¤Îbroadcast²ó¿ô¤òÁý¤ä¤¹Êý¤¬·ë²ÌÅª¤Ë¹âÂ®) */
/*3*/ for (CHIP=0; CHIP<NCHIP; CHIP++) { /* will be parallelized by multi-chip (M/#chip) */
...
\begin{verbatim}
  ...
  for (i=0; i<M+1; i++)
    p[i] = i;
  for (i=0; i<M; i++) { /* ÎóÊý¸þ */
    pmax = 0.0;
    k = -1;
    for (j=i; j<M; j++) { /* ¹ÔÊý¸þ¤ËÃµº÷ */
    ...
    if (len < 16) { /* len<1¤Ç¤âÀµ¾ï¤Ê¤Î¤ÇÀ­Ç½ºÇÂç²½¤Ç·è¤á¤Æ¤è¤¤ */
      for (j=i+1; j<M; j+=NCHIP*H*RMGRP) { /* ¹ÔÊý¸þ */
        for (CHIP=0; CHIP<NCHIP; CHIP++) {
          for (h=0; h<H; h++) { /* vertical (parallel) execution */
            for (grp=0; grp<RMGRP; grp++) {
              for (k=0; k<M-(i+1); k++) { /* ºÇÆâÎóÊý¸þ */
...
\begin{verbatim}
  ...
  for (i=0; i<M; i++) {
    for (j=0; j<M; j++)
      b[p[j]] = (i==j)?1.0:0.0;
    /*for (j=1; j<M; j++) { *//*ÄÌ¾ï¤ÎÏ¢Î©°ì»þÊýÄø¼°¤Î¾ì¹ç*/
    for (j=i+1; j<M; j++) { /* µÕ¹ÔÎó(b[]=E)¤Î¾ì¹ç,k<i¤Ç¤Ïb[]==0¤Ê¤Î¤Çj=i+1¤«¤é³«»Ï */
      /*for (k=0; k<j; k++) *//*ÄÌ¾ï¤ÎÏ¢Î©°ì»þÊýÄø¼°¤Î¾ì¹ç*/
...
\begin{verbatim}
  ...
  for (i=0; i<M; i+=NCHIP*H) { /* ÎóÊý¸þ */
    for (j=1; j<M; j++) { /* ¹ÔÊý¸þ */
      /********************************************/
      for (CHIP=0; CHIP<NCHIP; CHIP++) {
        for (k=0; k<j; k++) { /* ºÇÆâÎóÊý¸þ */
          for (h=0; h<H; h++) { /* vertical (parallel) execution */
            b[(i+CHIP*H+h)*M+j] -= A[p[j]*M+k]*b[(i+CHIP*H+h)*M+k];
                                        /* b[*]¤ò½Ä¤ËÇÛÃÖ¤¹¤ë¾ì¹ç,A¤â½ÄÇÛÃÖ. j¤¬ÎóÊý¸þ¤ËÂÐ±þ¤¹¤ë¤¬ÎóÊý¸þ¤Î°ÜÆ°¤Çk¤âÄ¹¤¯¤Ê¤ë */
                                        /* ²ÄÊÑÄ¹k¤òHÊý¸þ¤ËÅ¸³«¼ÌÁü¤¹¤ë¤Î¤ÏÆñ¤·¤¤.k¤Ïread-modify-write¤Î²óÅ¾¿ô¤Ë¼ÌÁü¤¹¤ë¤·¤«¤Ê¤¤ */
                                        /* b[*]¤ÈA[j][*]¤¬Æ±°ìLMM¤ËÆþ¤ëÁ°Äó ºÇÂç64KB/4/2=³Æ8KÍ×ÁÇ,b[*]¤ò¤¤¤«¤ËÆ°¤«¤µ¤Ê¤¤¤« */
                                        /* ²óÅ¾¿ôj¤ò°ìÀÆÅ¬ÍÑ¤¹¤ë¤Ë¤Ï,i¤òWxHÊý¸þ¤ËÅ¸³«¤¹¤ë¤Î¤¬¼«Á³ */
              /*           ¢­¡úA[p[j]][*]¤òbroadcast²ÄÇ½ ³ÆA[p[j]][*]¤Ïp[j]¤¬ÉÔÏ¢Â³¤Ê¤Î¤Ç1KÍ×ÁÇ¤Þ¤Ç¼ýÍÆ.¤Ä¤Þ¤êÆó½Å¥ë¡¼¥×Å¸³«¤ÏÌµÍý */
              /* +-----------------------------------+     +------+ +------+ +------+ */
...
\begin{verbatim}
  ...
  for (i=0; i<M; i+=NCHIP*H) { /* ÎóÊý¸þ */
    ...
    for (j=i+1; j<M; j++) { /* µÕ¹ÔÎó(b[]=E)¤Î¾ì¹ç,k<i¤Ç¤Ïb[]==0¤Ê¤Î¤Çj=i+1¤«¤é³«»Ï */
      ...
      Uint *topw = (Ull)top;
      /*Uint  len = (j+1)/2;*/
      Uint  len = j-i;/* b¤¬Ã±°Ì¹ÔÎó¤Î¾ì¹ç,k<i¤Ç¤Ïb[]==0¤Ê¤Î¤Çk=i¤«¤é³«»Ï */
      /********************************************/
      if (len < 2) { /* len<1¤Ç¤âÀµ¾ï¤Ê¤Î¤ÇÀ­Ç½ºÇÂç²½¤Ç·è¤á¤Æ¤è¤¤ */
        for (CHIP=0; CHIP<NCHIP; CHIP++) {
        /*for (k=0; k<j; k++) { *//*ÄÌ¾ï¤ÎÏ¢Î©°ì»þÊýÄø¼°¤Î¾ì¹ç*/
          for (k=i; k<j; k++) { /* µÕ¹ÔÎó(b[]=E)¤Î¾ì¹ç,k<i¤Ç¤Ïb[]==0¤Ê¤Î¤Çk=i¤«¤é³«»Ï */
            for (h=0; h<H; h++) { /* vertical (parallel) execution */
...
\begin{verbatim}
  for (i=0; i<M; i+=NCHIP*H) { /* ÎóÊý¸þ */
    for (j=M-1; j>=0; j--) { /* ¹ÔÊý¸þ */
      /********************************************/
      for (CHIP=0; CHIP<NCHIP; CHIP++) {
        for (k=M-1; k>j; k--) { /* ºÇÆâÎóÊý¸þ */
          for (h=0; h<H; h++) { /* vertical (parallel) execution */
            b[(i+CHIP*H+h)*M+j] -= A[p[j]*M+k]*x[(i+CHIP*H+h)*M+k];
                                            /* x[*]¤ÈA[j][*]¤¬Æ±°ìLMM¤ËÆþ¤ëÁ°Äó ºÇÂç64KB/4/2=³Æ8KÍ×ÁÇ,x[*]¤ò¤¤¤«¤ËÆ°¤«¤µ¤Ê¤¤¤« */
                                            /* ²óÅ¾¿ôj¤ò°ìÀÆÅ¬ÍÑ¤¹¤ë¤Ë¤Ï,i¤òWxHÊý¸þ¤ËÅ¸³«¤¹¤ë¤Î¤¬¼«Á³ */
            /*           ¢­¡úA[p[j]][*]¤òbroadcast²ÄÇ½ ³ÆA[p[j]][*]¤Ïp[j]¤¬ÉÔÏ¢Â³¤Ê¤Î¤Ç1KÍ×ÁÇ¤Þ¤Ç¼ýÍÆ.¤Ä¤Þ¤êÆó½Å¥ë¡¼¥×Å¸³«¤ÏÌµÍý */
            /* +-----------------------------------+     +------+ +------+ +------+ */
      ...
      for (CHIP=0; CHIP<NCHIP; CHIP++) {
        for (h=0; h<H; h++) { /* vertical (parallel) execution */
          inv1[j*M+p[i+CHIP*H+h+w]] = x[(i+CHIP*H+h)*M+j] = A[p[j]*M+j]*b[(i+CHIP*H+h)*M+j]; /* PIO¤Ë¤ÆLMM¤Îx[i*M+j]¤òÄ¾ÀÜ¹¹¿· */
                                                                                             /* i¤Ï¤½¤Î¤Þ¤Þ¤Ç,j¤òÀÚÂØ¤¨ */
        }
      }
    }
...
//EMAX5A drain_dirty_lmm
      ...
      for (CHIP=0; CHIP<NCHIP; CHIP++) {
        for (h=0; h<H; h++) { /* vertical (parallel) execution */
          inv1[j*M+p[i+CHIP*H+h]] = x[(i+CHIP*H+h)*M+j] = A[p[j]*M+j]*b[(i+CHIP*H+h)*M+j]; /* PIO¤Ë¤ÆLMM¤Îx[i*M+j]¤òÄ¾ÀÜ¹¹¿· */
                                                                                           /* i¤Ï¤½¤Î¤Þ¤Þ¤Ç,j¤òÀÚÂØ¤¨ */
        }
      }
    } /* j-loop */
...
#if !defined(EMAX5) && !defined(EMAX6)
    for (j=0; j<p->nedges; j++) {                      /* R£°ÃÊ:ºÇÆâ¥ë¡¼¥×256²óÅ¾ÄøÅÙ */
      n = p->npage[j/MAXNV_PPAGE]+(j%MAXNV_PPAGE);
      q = n->vp;                                       /* R£°ÃÊ:neighborvertexÁ´ÂÎ¤òÇÛÃÖ pointer¤ò»È¤¤»²¾È */
      qid = n->id;                                     /* R£°ÃÊ:Æ±¾å */
      if (!q->parent) {                                /* R£±ÃÊ:vertexÁ´ÂÎ¤òÇÛÃÖ pointer->pointer¤ò»È¤¤»²¾È */
        /************************/
        while (cmpxchg(&Sem0, -1, param->th) != -1);
        /************************/
        if (!q->parent) {                              /* R£±ÃÊ:Æ±¾å */
          if (nnextfrontiers >= MVL) {
            printf("vlist[%d] exhausted\n", MVL);
            exit(1);
          }
          q->parent = pid;                             /* W£²ÃÊ:verex¹¹¿· */
          q->depth  = depth;                           /* W£²ÃÊ:Æ±¾å */
          q->findex = nnextfrontiers;                  /* W£²ÃÊ:Æ±¾å */
          nextfrontier[nnextfrontiers] = q;            /* W£²ÃÊ:next_frontier[]¹¹¿· */
          nnextfrontiers++;                            /* W£²ÃÊ:Æ±¾å */
      ...
      else if (q->depth==depth-1 && q->findex<i) {     /* R£±ÃÊ:vertexÁ´ÂÎ¤òÇÛÃÖ pointer->pointer¤ò»È¤¤»²¾È */
        /************************/
        ...
        frontier_edge[nfrontier_edges].src = (pid<qid)?p:q; /* W£²ÃÊ:frontier_edge[]¹¹¿· */
        frontier_edge[nfrontier_edges].dst = (pid<qid)?q:p; /* W£²ÃÊ:Æ±¾å */
        nfrontier_edges++;                                  /* W£²ÃÊ:Æ±¾å */
...
void tri_kernel1(struct param_tricount *param)
{
  ...
  sdepth = p->depth;
    ...
    for (j=0; j<p->nedges; j++) {                    /* R£°ÃÊ:ºÇÆâ¥ë¡¼¥×256²óÅ¾ÄøÅÙ */
      ...
      q = n->vp;                                     /* R£°ÃÊ:neighborvertexÁ´ÂÎ¤òÇÛÃÖ pointer¤ò»È¤¤»²¾È */
      qid = n->id;                                   /* R£°ÃÊ:Æ±¾å */
      tdepth = q->depth;                             /* R£±ÃÊ:vertexÁ´ÂÎ¤òÇÛÃÖ pointer->pointer¤ò»È¤¤»²¾È */
      if ((tdepth==sdepth-1)||(tdepth==sdepth+1)||(tdepth==sdepth && qid<pid)) { /* R£²ÃÊ:Èæ³Ó */
        if (search_nvertex(t->nhashtbl, qid))        /* R£³ÃÊ:HASH-SEARCH/CAM-SEARCH */
          tricount++;                                /* W£´ÃÊ:¥«¥¦¥ó¥¿¹¹¿· */
      }
    }
...
for (i=0; i<ctx->mbuflen; i+=BLKSIZE) { /* 1¥Ç¡¼¥¿Î®Æâ¤ÎÊÂÎó¼Â¹Ô¤ÏÉÔ²ÄÇ½. Â¿¿ô¥Ç¡¼¥¿Î®¤Î¥Ñ¥¤¥×¥é¥¤¥ó¼Â¹Ô¤Î¤ß */
...
for (i=0; i<ctx->mbuflen; i+=BLKSIZE) { /* 1¥Ç¡¼¥¿Î®Æâ¤ÎÊÂÎó¼Â¹Ô¤ÏÉÔ²ÄÇ½. Â¿¿ô¥Ç¡¼¥¿Î®¤Î¥Ñ¥¤¥×¥é¥¤¥ó¼Â¹Ô¤Î¤ß */
  ...
  /* a,b,c,d,e,f,g,h ¤òLMM·ÐÍ³¤Çctx.state¤Ë°ìÃ¶¥¹¥È¥¢¤·¡¤·ÑÂ³¼Â¹Ô */
  /* state[8]¤ò³Æ¥Ç¡¼¥¿Î®¤Ë¥¢¥µ¥¤¥ó¤·¤ÆÆó¼¡¸µÇÛÎó²½.CGRA¤È¤·¤Æ¥Ñ¥¤¥×¥é¥¤¥ó½èÍý */
...
mop(OP_STR, 3, &hd,        sregs6, th,        MSK_W0, sregs,     MAX_THNUM*8, 0, 0,      NULL,    MAX_THNUM*8)
  ...
  for (INIT1=1,LOOP1=DIV,j=0; LOOP1--; INIT1=0,j+=BLKSIZE/DIV*4) {
    mptop = (LOOP1==0)? mtop+MAX_THNUM*BLKSIZE*4:mtop; /* ºÇ½ª²ó¤Î¤ßPLOAD */
//with-prefetch
//EMAX5A begin imax mapdist=0
 /*3*/for (CHIP=0; CHIP<NCHIP; CHIP++) {
