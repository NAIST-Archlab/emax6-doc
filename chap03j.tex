...
LMM=8K*4byte*2=64KBの場合は最大8192要素，
LMM=64K*4byte*2=512KBの場合は最大65536要素のFFT．
...
FFTのパイプライン版．LMM=4K*4byte*2*DB=64KBの場合は最大4096要素，
LMM=32K*4byte*2*DB=512KBの場合は最大32768要素のFFT．中間のLMMをダブルバッファ
として使用している．
...
LMM=4K*8byte*DB=64KBの場合は最大4096要素，LMM=32K*8byte*DB=512KBの場合は最大
32768要素のパイプライン版merge sort．中間のLMMをダブルバッファとして使用して
いる．なお，入力データは各要素8バイトである．上位4バイトが大小比較対象，下位
4バイトはポインタや属性など任意の値に使用できる．
...
Unaligned loadとSIMD演算を使う3D-CNN
